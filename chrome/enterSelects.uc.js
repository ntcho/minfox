...
UC.enterSelects = {
  ...
  init: function () {
    ...
    this.orig_isDeferringEvents = Object.getOwnPropertyDescriptor(
      this.eventBuffer,
      "isDeferringEvents"
    ).get;
  ...
  exec: function (win) {
    ...
    if (win.__SSi) this.addUrlbarListeners(win);
    else Services.obs.addObserver(observe, "browser-window-before-show");
  },
  ...
  controller: ChromeUtils.importESModule("resource:///modules/UrlbarController.sys.mjs")
    .UrlbarController.prototype,
  eventBuffer: ChromeUtils.importESModule(
    "resource:///modules/UrlbarEventBufferer.sys.mjs"
  ).UrlbarEventBufferer.prototype,
  input: ChromeUtils.importESModule("resource:///modules/UrlbarInput.sys.mjs")
    .UrlbarInput.prototype,
  queryContext: ChromeUtils.importESModule("resource:///modules/UrlbarUtils.sys.mjs")
    .UrlbarQueryContext.prototype,

  shouldSelect: function (gURLBar, queryContext) {
    if (gURLBar.searchMode?.engineName || queryContext.results.length < 2) return false;
    ...
    if (!this.AUTOSELECT_WITH_SPACE && /\s/.test(search)) return false;

    // Test about:about, chrome://blabla ...
    if (/[a-zA-Z][\w-]*:(\/\/)?[\w-]+/.test(search)) return false;

    // Check if the first word is a keyword (search or bookmark)
    if (!!queryContext.results[0].payload.keyword) return false;
  ...
  keyD: function (e) {
    ...
    if (e.keyCode == e.DOM_VK_TAB) {
      ...
      if (
        ...
        new RegExp(
          /^(https?:\/\/(www\.)?)?/.source +
            gURLBar.value.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")
        ).test(url) &&
        gURLBar.value != url.match(/(\w*:\/\/)?[^/]+\//)[0]
      ) {
        gURLBar.inputField.value = gURLBar._untrimmedValue = gURLBar
          ._trimValue(url.match(/(\w*:\/\/)?[^/]+\//)[0] + "/")
          .slice(0, -1);
        gURLBar.view.close();
