...
UC.rebuild = {
  ...
  onpopup: function (event) {
    ...
    let mi = event.target.appendChild(this.elBuilder(document, 'menuitem', {
      ...
      command: () => xPref.set(_uc.PREF_ENABLED, !enabled),
      type: 'checkbox',
      checked: enabled
    }));
    ...
    Object.values(_uc.scripts).sort((a, b) => a.name.localeCompare(b.name)).forEach(script => {
      ...
      mi = event.target.appendChild(this.elBuilder(document, 'menuitem', {
        ...
        click: event => UC.rebuild.clickScriptMenu(event),
        mouseup: event => UC.rebuild.shouldPreventHide(event),
        type: 'checkbox',
  ...
  createButton (aDocument) {
    ...
    let mp = UC.rebuild.elBuilder(aDocument, 'menupopup', {
      ...
      popupshowing: event => UC.rebuild.onpopup(event),
    ...
    let mi1 = UC.rebuild.elBuilder(aDocument, 'menuitem', {
      ...
      command: function(){Services.dirsvc.get("UChrm", Ci.nsIFile).launch()},
    });
    mg.appendChild(mi1);
    ...
    let tb = UC.rebuild.elBuilder(aDocument, 'menuitem', {
      ...
      command: function() {UC.rebuild.restart()},
    });
    mg.appendChild(tb);
    ...
    let mi2 = UC.rebuild.elBuilder(aDocument, 'menuitem', {
      ...
      command: function(){UC.rebuild.toggleUI()},
    });
    mp2.appendChild(mi2);
  ...
  createPanel (aDocument) {
    ...
    if (viewCache) {          
      ...
      subviewBody.appendChild(UC.rebuild.createMenuItem(aDocument, 'openChrome', 'url(chrome://browser/skin/folder.svg)', 'Open chrome directory', function(){Services.dirsvc.get('UChrm', Ci.nsIFile).launch()}));
      subviewBody.appendChild(UC.rebuild.createMenuItem(aDocument, 'restart', 'url(chrome://browser/skin/reload.svg)', 'Restart ' + _uc.BROWSERNAME, function(){UC.rebuild.restart()}));
      subviewBody.appendChild(aDocument.createXULElement('toolbarseparator'));
      const enabledMenuItem = UC.rebuild.createMenuItem(aDocument, 'enabled', null, 'Enabled', function(){xPref.set(_uc.PREF_ENABLED, !!this.checked)});
      ...
      scriptsButton.addEventListener('command', function() { aDocument.ownerGlobal.PanelUI.showSubView('appMenu-userChromeJsView', this)});
